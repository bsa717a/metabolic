#!/usr/bin/env bash
# Provision Cloud Scheduler for the transactional email queue (welcome emails, retries).
...
JOB_NAME="${EMAIL_SCHEDULER_JOB:-metabolic-email-tick}"
...
fail() {
  ...
  if [[ "$BEST_EFFORT" == "true" ]]; then
    ...
    echo "  Run once with admin credentials: ./scripts/setup-email-scheduler.sh" >&2
    exit 0
  fi
  exit 1
}
...
SCHEDULER_ARGS=(
  --location "$REGION"
  --schedule "* * * * *"
  --uri "${API_URL}/api/internal/email/tick"
  --http-method POST
...
if gcloud scheduler jobs describe "$JOB_NAME" --location "$REGION" >/dev/null 2>&1; then
  ...
  echo "Updated Cloud Scheduler job: $JOB_NAME -> ${API_URL}/api/internal/email/tick"
else
  ...
  echo "Created Cloud Scheduler job: $JOB_NAME -> ${API_URL}/api/internal/email/tick"
fi
