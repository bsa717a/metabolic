...
export function CopyDayForward({
  ...
  async function copy() {
    ...
    setError(null);
    try {
      await api(`/api/daily-logs/${date}/copy-forward`, {
        method: 'POST',
  ...
  return (
    <div ref={ref} className="absolute right-1 top-1/2 -translate-y-1/2">
      <button
        ...
        className="grid h-6 w-6 place-items-center rounded-full bg-brand-green/15 text-brand-green transition hover:bg-brand-green/25"
      >
      ...
      {open && (
        <div
          className="absolute right-0 top-9 z-50 w-60 rounded-2xl border border-app-border bg-app-surface p-3 text-left shadow-lg"
