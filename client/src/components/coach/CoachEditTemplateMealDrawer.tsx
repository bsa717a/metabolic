...
export function CoachEditTemplateMealDrawer({
  open,
  meal,
  onClose,
  onSaved
  ...
  async function addFood(food: Food) {
    ...
    setAddError('');
    try {
      await api(`/api/coach/nutrition-template-meals/${meal.id}/items`, {
        method: 'POST',
        body: JSON.stringify({
          foodId: food.id,
  ...
  async function updateQuantity(item: NutritionTemplateMealItem, quantity: number) {
    ...
    await api(`/api/coach/nutrition-template-meal-items/${item.id}`, {
      method: 'PATCH',
      body: JSON.stringify({
        quantity,
  ...
  async function removeItem(itemId: string) {
    await api(`/api/coach/nutrition-template-meal-items/${itemId}`, { method: 'DELETE' });
    onSaved();
  }
        ...
        <div>
          ...
          {items.length === 0 ? (
            ...
            <ul className="space-y-2">
              {items.map((item) => (
                <li key={item.id} className="flex items-center justify-between gap-2 rounded-2xl bg-app-muted p-3">
