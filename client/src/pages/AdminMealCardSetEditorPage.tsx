      ...
      {set.cards.map((card, cardIndex) => (
        <Card key={card.id} className="space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" title="Move up" onClick={() => void call(`/api/admin/cards/${card.id}/move`, 'POST', { direction: 'up' })} className="text-app-text-muted hover:text-app-text"><ArrowUp size={16} /></button>
            <button type="button" title="Move down" onClick={() => void call(`/api/admin/cards/${card.id}/move`, 'POST', { direction: 'down' })} className="text-app-text-muted hover:text-app-text"><ArrowDown size={16} /></button>
            <button
              type="button"
              title="Delete card"
              onClick={() => window.confirm(`Delete the "${card.name}" step and all its options?`) && void call(`/api/admin/cards/${card.id}`, 'DELETE')}
